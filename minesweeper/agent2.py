...
def sweep_grid(grid, mineCount):
    ...
    while True:
        ...
        if not info and not discovered:
            ...
            if kb != []:
                #print("kb: ", kb)
                (i, j), min_val = best_guess_easy(kb)
                default_val = (numpy.count_nonzero(grid == -1) - numpy.count_nonzero(mines == -1) ) / numpy.count_nonzero(mines == 0)
                assert(default_val >= 0)
               # print(min_val)
                if (default_val < min_val):
                    #print(default_val, min_val, "error")
                    #print((d - numpy.count_nonzero(mines == -1) ))
...
def best_guess_easy(knowledge_base):
    """Given a knowledge base and a graph, find the best guess (one least likely to be a mine)
    """
    probability_max = dict()
    for fact in knowledge_base:
        chance = fact.get_value() / len(fact.get_unknowns())
        for unknown in fact.get_unknowns():
            probability_max.update({unknown : max(probability_max.get(unknown) or chance, chance)}) #set each to max
            #probability_max.update({unkown : chance})
    return min(probability_max.items())
    #return min(probability_max, key = probability_max.get), min_val
    #return [k for k, v in enumerate(probability_max) if v == min_val][0]

def best_guess_hard(knowledge_base):
    """Given a knowledge base and a graph, find the best guess (one least likely to be a mine)
    """
    ones = {}
    zeros = {}
