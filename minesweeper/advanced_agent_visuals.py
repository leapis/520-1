...
import minesweeper_visuals as minesweeper
import random
import inference
...
def sweep_grid(grid):
    ...
    _, axis_grid = pyplot.subplots()
    axis_grid.matshow(grid)
    ...
    for i in range(d):
        for j in range(d):
            ...
            if entry == "-1":
                entry = "M"
            axis_grid.text(i, j, entry, va='center', ha='center')

    explosions = 0
    clues = numpy.zeros((d, d), dtype=int)
    mines = numpy.zeros((d, d), dtype=int) # 0 for covered, -1 for mine, 1 for safe

    while True:
        info = False # Whether any cell moved forward in the puzzle
        for i in range(0, d):
            for j in range(0, d):
                truth = inspect_cell(grid, mines, clues, i, j)
                info = truth or info
                if truth:
                    pyplot.matshow(clues)
...
if __name__ == '__main__':
    ...
    grid = minesweeper.generate_environment(7, 5)
    sweep_grid(grid)
